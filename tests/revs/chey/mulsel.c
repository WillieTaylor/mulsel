...
typedef struct  {
        ...
        Prot;
Prot  *pair_list; // for use in repeated call in count() (seqsort.c)
Prot  *count();
...
FILE	*win;
	...
	nwords = 0;
	win = fopen("keywords.dat","r");
	if (win) { // read a file of penalty:keword pairs
		while (1) { int io;
			io = read_line(win,line);
			if (io==0) continue;
			if (io < 0) break;
			words[nwords] = (char*)malloc(sizeof(char)*99);
			sscanf(line,"%d %s", costs+nwords,words[nwords]);
			nwords++;
		}
		fclose(win);
	}
	...
	{ int	*pep_order;
	  int	i;
		pep_order = (int*)malloc(sizeof(int)*(nblocks+1)); TEST(pep_order)
		// convert sequences into peptide lists (return list size in pep_order[])
		pept_out(nblocks,pep_order);
		for (i=0; i<nblocks; i++) block[i]->seq[0]->odd = pep_order[i+1];
		// compare peptide lists (as a series of <pep_span> blocks)
		npairs = 0;
		for (i=0; i<abs(pep_span); i++)
		{ int	aspan, bspan, minp, hold, diag, chunk = nblocks/pep_span,
			io = read_line(run_data,line);
			sscanf(line,"%d%d%d%d%d", &aspan,&bspan,&minp,&hold,&diag);
			if (aspan < 0) aspan = chunk* i;
			if (bspan < 0) bspan = chunk*(i+1);
			if (i+1==abs(pep_span)) bspan = nblocks;
			// load up hits in pept_list[] for current chunk
			pair_list = count(i,nblocks,aspan,bspan,minp,hold,diag,pair_list,&npairs);
		}
		// init list of ordered sequences
		for (i=0; i<nblocks; i++) pep_order[i] = i;
		// cluster using list of pairs gathered in pair_list[]
		pre_order(pep_order);
	...
	{ 	Blocks	*b = block[i], *b0 = block[0];
		...
		{ Seqs	*s = b->seq[j];
		  ...
		  float resol, bvalue;
			...
			if (seedlen && (s->length > mean)) resol *= 0.5;
                        resol -= log((float)s->odd); // odd = No of peptideis, low = bad
			for (k=0; k<nwords; k++) { // apply user penalties
                        	if (strstr(s->title,words[k])) {
					resol += costs[k];
				}
			}
                        if (strstr(s->code,"SEQ")) nat++;
                        if (strstr(s->code,"REV")) rev++;
			if (nat>2 && rev>2) {
				// Pt(SEQ and REV together) NL
				// exit(1);
			}
                        ...
                        if (strstr(s->title,"hypothetical")) resol += 5.0;
                        if (strstr(s->title,"UNCHARACRERI")) resol += 5.0;
                        if (strstr(s->title,"uncharacteri")) resol += 5.0;
...
{	Blocks	*b;
	...
	if (delayed==1) {
		...
		if (seq.name[5]=='>') s->odd = (int)copy_seq(key, s->code, seq.seq, s->residue,  seq.len);
		if (seq.name[5]=='<') s->odd = (int)copy_seq(key, s->code, seq.seq, s->residue, -seq.len);
	}
	if (seq.name[5]=='<') s->code[5] = '>'; // stops re-reversal of final.seq on second cycle
}
...
int	rev = 0;
	if (length < 0) { rev = 1; length = -length; }
	for(j=1; j <= length; j++) 
	{ char	res, upr;
		if (rev==0) res = res_in[j-1]; else res = res_in[length-j];
		upr = UPPER(res);
		if (rev==0 && res=='*') {
			length = j-1;
			return;
...
int 	key, maxs, mins, in;
{
	...
	seq.len = 0;
	while (1) { int in, io = read_line(user,line);
		if (io==0) continue;
		if (io<0) break;
		in = strncmp(line,"SEED:",5);	// = 0 if no match
		if (*line == '>') in = 1;	// normal seq
		if (*line == '<') in = 1;	// reversed
		if (!in) continue;
		// if here, line starts with '>' or "SEED:" (ie valid sequence start)
		if (strncmp(line+1,"SKIP",4)==0) continue; // skip this sequence
...
int	*pep_order;
{
	...
	printf(" malloc OK\n");
	reorder(pep_pair); // sorts on the pept_list[].count of pairs in seqsort.c
	for (n=0; n<npairs; n++) {
...
int	*pairs; // hashed pair IDs
{
	...
	printf(" malloc OK");
	for (i=0; i<npairs; i++) score[i] = (float)pair_list[i].count;
